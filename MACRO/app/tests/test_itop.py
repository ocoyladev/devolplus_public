...
def test_itop_descarga_4ta_lanza_job() -> None:
    app = create_app()
    fake = FakeJobs()
    app.state.jobs = fake
    client = TestClient(app)
    resp = client.post(
        "/api/itop/descarga", json={"tipo": "4ta", "row": {"num_doc": "D1"}}
    )
    assert resp.status_code == 200
    assert fake.calls == [("itop_4ta", "D1")]


def test_itop_modificar_lanza_job() -> None:
    app = create_app()
    fake = FakeJobs()
    app.state.jobs = fake
    client = TestClient(app)
    ...
    assert resp.status_code == 200
    assert fake.calls == [("itop_modificar", "D2")]


def test_empleadores_601_devuelve_lista() -> None:
    with patch(
        "MACRO.flujos.flujo_itop.extraer_empleadores_601",
...
def test_itop_descarga_tipo_invalido_rechazado() -> None:
    app = create_app()
    app.state.jobs = FakeJobs()
    client = TestClient(app)
    resp = client.post(
        "/api/itop/descarga", json={"tipo": "9na", "row": {"num_doc": "D1"}}
    )
    assert resp.status_code == 422


def test_itop_preview_4ta() -> None:
    client = TestClient(create_app())
    resp = client.post("/api/itop/preview", json={
        "tipo": "4ta",
...
def test_itop_preview_601_completo_usa_empleador() -> None:
    client = TestClient(create_app())
    resp = client.post("/api/itop/preview", json={
        "tipo": "601_completo",
